...
import {Image, Layout, Space, Table, Typography} from '@douyinfe/semi-ui';
...
const Passport: NextPage = () => {
  ...
  const [diffSwitch, setDiffSwitch] = useState(false);
...
const Item = (props: { record: any, tempSearch: string[] }) => {
  ...
  return <div
    ...
    <Space>
      <Image src={`/next-image/country/${record.iso?.toLowerCase()}.svg`} width={16} preview={false}/>
      <div>{record.name}</div>
    </Space>
